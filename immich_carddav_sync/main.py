...
import uuid
import vobject
from immichpy.client.generated import PeopleResponseDto, PeopleUpdateDto, PeopleUpdateItem, BulkIdResponseDto
...
async def set_immich_birth_date(person_id: uuid.UUID, birth_date: str, api_url: str, api_key: str):
    ...
    async with AsyncClient(base_url=api_url, api_key=api_key) as client:
        dto = PeopleUpdateDto(
            people=[PeopleUpdateItem(birthDate=datetime.date.fromisoformat(birth_date), id=person_id)]
        )
        ...
        if match:
            for person in people_data:
                ...
                if match["birthday"] != person[1]:
                    ...
                    logger.info(
                        "Birth date for person %s (%s) is already %s - skipping"
